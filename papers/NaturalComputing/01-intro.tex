...
After fixing the strictly necessary background on RS (\S~\ref{sec:RS}) and GROOVE (\S~\ref{sec:GTS}), we illustrate in \S~\ref{sec:RS2GTS} the overall rationale, blueprint and main features of our implementation  by exploiting a small running example that models a vending machine (\S~\ref{sec:student}).
The subsequent experimentation (\S~\ref{sec:experiments}) is concerned with revisiting previous case studies from the literature on RS to see how GROOVE can be used to enhance their analysis. 
In particular, we tackle 
the comorbidity treatment analysis from~\cite{DBLP:conf/cmsb/BowlesBBFGM24} in \S~\ref{sec:cmsb2024}, 
the protein signaling networks analysis from~\cite{DBLP:conf/cmsb/BallisBFO24} in \S~\ref{sec:ccReact}, and
the T cell differentiation analysis from~\cite{datamod2023} in \S~\ref{sec:datamod2023}.
