...
\subsection{Reaction Systems with Guarded Contexts}\label{sec:RS}

First, we briefly account for the classical set theoretic definition of Reaction Systems (RSs)~\cite{DBLP:journals/fuin/EhrenfeuchtR07}. Then, we focus on their process algebraic version~\cite{DBLP:journals/tcs/BrodoBF21} and its further extension with guarded contexts in~\cite{DBLP:conf/cmsb/BowlesBBFGM24}. Notably, the process algebraic semantics matches the set theoretic dynamics of RSs (as formalized by~\cite[Th.~19]{DBLP:journals/tcs/BrodoBF21}). 
