...
The Boolean network model is graphically represented as shown in Fig.~\ref{fig:model-graph}, where the 9 orange nodes represent different environmental stimuli that the T-cell can receive; all the other nodes represent so-called \emph{transcription factors}\todo{A: Is this correct?} and have an associated Boolean update formula that specifies when they are triggered. For the readers' convenience, all update formulas are reported in Fig.~\ref{fig:boolean-formulas} in the Appendix. 
...
T-cells can manifest four phenotypes, \texttt{Th1}, \texttt{Th2}, \texttt{Th17} and \texttt{iTreg}, represented by the four transcription factors \texttt{Tbet}, \texttt{GATA3}, \texttt{RORgt} and \texttt{Foxp3}, respectively.\todo{A: this suggests it's a one-to-one correspondence.}
...
We have selected this case study because it shows the applicability o four method to Boolean networks models, like those available in the public database on the CellCollective platform~\cite{helikar2012cell}. 
