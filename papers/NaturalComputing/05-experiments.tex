...
For these models, the most relevant analyses are often reachability (e.g., which phenotypes are reachable) and causality (what is the effect of environmental conditions?).
...
The RS context chooses in a non-deterministic way the set of environmental stimuli that will persist; that is, for each possible stimulus $s$ we define the context processes
\[
\begin{array}{rllrll}
...
%\end{array}
%\]
Correspondingly, in the case study, the LTS has an initial branching into $2^9$ different states, because there are $9$ possible stimuli to be considered. Subsequently, a deterministic computation will start, leading to attractors for each of the initial $2^9$ choices. 
