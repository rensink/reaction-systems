...
The Boolean network model is graphically represented as shown in Fig.~\ref{fig:model-graph}, where the 9 orange nodes represent different environmental stimuli that the T cell can receive. %Then, it contains 23 nodes for relevant T cell genes, and 6 nodes for secreted cytokines.
...
Their corresponding RSs are concerned just with a special kind of nondeterministic persistent contexts, where at the beginning of the experiment a subset of external stimuli is chosen and then provided at each subsequent step, leading inevitably the RS to exhibit a looping behaviour (called attractor).
...
The RS context chooses in a non-deterministic way the set of environmental stimuli that will persist, i.e., for each possible stimulus $s$ we define the context processes
\[
\begin{array}{rllrll}
\mathsf{X}_s & \triangleq & \{s\}.\mathsf{X}'_s + \mathsf{Emp} &\quad
\mathsf{X}'_s & \triangleq & \{\mathsf{s}\}.\mathsf{X}'_s
\end{array}
\]
and then we compose all $\mathsf{X}_s$  in parallel (for all $s$).
%\[
%\begin{array}{rllrll}
...
%\end{array}
%\]
Correspondingly, in the case study, the LTS has an initial branching into $2^9$ different states, because there are $9$ possible stimulus to be considered. Subsequently, a deterministic computation will start, leading to some attractors. 

\medskip\noindent\textbf{Analysis goals.}
...
\medskip\noindent\textbf{Previous approach.}
...
\medskip\noindent\textbf{GROOVE experimentation.}
