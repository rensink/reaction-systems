...
\textcolor{blue}{
As observed in Remark~\ref{rem:shortlts}, any RS process $[\mathsf{M}]$ can be written in the form $[\mathsf{Rs}~|~\mathsf{Ks}~|~D]$ where:\todo{AR: So then there is only one such set $D$? This would mean that reason (i) below for non-isomorphism no longer holds.}
\begin{enumerate*}[label=\emph{(\roman*)}]
