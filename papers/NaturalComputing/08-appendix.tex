...
We claim that this equivalence establishes a bisimulation between the \GROOVE and RS state spaces. (It is not an isomorphism, because the RS semantics for $\textsf K$ ``consumes'' the process (rules \textit{(Cxt)}--\textit{(Rec)} in \Cref{fig:guardforRS2nd}) whereas $G$ keeps the automaton intact, merely moving the \current pointer.)

To prove this claim, we have to show correspondence of the transitions. Concretely, to any transition carrying the label $\obs{\obs{D}{R',I',C}}{R,I,P}$, there corresponds a single application of the recipe \fireR, which in turn consists of applications of rules \contextR (see \Cref{fig:context}) followed by \reactR (see \Cref{fig:react}).
...
\Cref{tab:mex} shows the \GROOVE performance in generating the full state space of the mutual exclusion example of \cite{DBLP:journals/isci/MeskiPR15,DBLP:journals/fuin/NobilePSMCMB17}, discussed in \Cref{sec:related}.
...
\hline
\bf Case & \multicolumn{2}{c|}{\bf States} & \multicolumn{2}{c|}{\bf Time} & \multicolumn{2}{c|}{\bf Memory} \\
\multicolumn{1}{|c|}{\bf \#} & \multicolumn{1}{c@{{\;}}}{\bf \#} & \multicolumn{1}{@{}c|}{\bf ratio} & \multicolumn{1}{c@{{\;}}}{\bf s} & \multicolumn{1}{@{}c|}{\bf ratio} & \multicolumn{1}{c@{{\;}}}{\bf MB} & \multicolumn{1}{@{}c|}{\bf ratio} \\
\hline
 2 &    11 &     &      0.2 &     &      1 &     \\
...
\end{verbatim}
\end{minipage}
\caption{\BioResolve implementation of the protein signalling network case study from \Cref{sec:ccReact}.}
